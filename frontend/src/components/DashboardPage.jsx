...
export default function DashboardPage({ currentUser }) {
  ...
  if (!stats) {
    return <div className="text-xs py-4" style={{ color: 'var(--text-tertiary)' }}>Loading dashboard...</div>
  }

  const summaryCards = [
    { label: 'TOTAL NICHES', value: stats.total_niches, color: 'var(--text-tertiary)' },
    { label: 'RECOMMENDATIONS', value: stats.recommendations_generated, color: 'var(--text-tertiary)' },
    { label: 'SESSIONS', value: stats.total_sessions, color: 'var(--text-tertiary)' },
    ...
    { label: 'MOST COMMON CATEGORY', value: stats.most_common_category || '—', color: 'var(--text-secondary)' },
    { label: 'TOP SAVED NICHE', value: stats.top_saved_niche || '—', color: 'var(--text-muted)' },
  ]

  return (
    <div>
      <div className="text-xs font-medium mb-4" style={{ color: 'var(--text-tertiary)' }}>USER DASHBOARD</div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mb-6">
        {summaryCards.map(c => (
          <div key={c.label} className="border p-3" style={{ borderColor: 'var(--border)', backgroundColor: 'var(--surface)' }}>
            <div className="text-xs mb-1 truncate" style={{ color: 'var(--text-tertiary)' }}>{c.label}</div>
      ...
      {stats.history_timeline && stats.history_timeline.length > 0 && (
        <div>
          <div className="text-xs font-medium mb-3" style={{ color: 'var(--text-tertiary)' }}>RECENT ACTIVITY</div>
            ...
            {stats.history_timeline.slice(0, 10).map((h, i) => (
              <div key={i} className="flex items-center justify-between px-4 py-2 text-xs" style={{ borderBottom: '1px solid #141414' }}>
                <div className="min-w-0 flex-1">
                  ...
                  <span className="ml-2" style={{ color: 'var(--text-tertiary)' }}>· {h.platform}</span>
                </div>
                <div className="flex items-center gap-3 flex-shrink-0 ml-3">
                  <span style={{ color: 'var(--text-tertiary)' }}>{h.tag_count} tags</span>
                  <span style={{ color: 'var(--text-tertiary)' }}>{h.confidence?.toFixed(0)}%</span>
                  <span style={{ color: 'var(--text-muted)' }}>{new Date(h.created_at).toLocaleDateString()}</span>
                </div>
              </div>
            ))}
          </div>
