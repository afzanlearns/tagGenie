export default function EmptyState({ hasHistory, onDemoSelect }) {
  return (
    <div className="border p-8 mt-4 text-center" style={{ borderColor: 'var(--border)' }}>
      <div className="text-lg mb-2" style={{ color: 'var(--border-light)' }}>◆</div>
      <h2 className="text-sm font-medium mb-2" style={{ color: 'var(--text)' }}>No recommendations generated yet</h2>
      <p className="text-xs mb-4 max-w-md mx-auto" style={{ color: 'var(--text-tertiary)' }}>
      ...
      {!hasHistory && (
        <div className="flex items-center justify-center gap-2 text-xs" style={{ color: 'var(--text-tertiary)' }}>
          ...
          <button
            ...
            className="px-2 py-1" style={{ backgroundColor: 'transparent', border: '1px solid #333', color: 'var(--text-secondary)', cursor: 'pointer' }}
          >
            Fleet Safety
          </button>
          <button
